...
def analyze():
    ...
    if ip_input:
        try:
            # Validate IP before sending
            ipaddress.ip_address(ip_input)  # will raise ValueError if invalid
            ...
            if data.get('status') == 'fail':
                result += f"<b>IP Tracking Error:</b> {data.get('message', 'Unknown error')}<br>"
            else:
                result += f"🔍 IP Address: {ip_input}<br>"
                result += f"📍 Country: {data.get('country', 'N/A')}<br>"
                result += f"🏙 Region: {data.get('regionName', 'N/A')}<br>"
            ...
            result += f"<b>IP Tracking Error:</b> Invalid IP format<br>"
        except Exception as e:
            result += f"<b>IP Tracking Error:</b> {e}<br>"
